...
const App = () => {
    ...
    */
		setLocation(null);
		const theFolder = allData.folders.filter(fold => fold.folderid === location);
						...
						const newNotifs = [
							{
								...
								title: "Last Attempt",
								text:
									"You have attempted to unlock your notes 3 times. One more failed attempt and the app will close and you will be logged out for your security",
								actions: [{ text: "close", func: () => setSystemNotifs([]) }]
							}
						];
						setSystemNotifs(newNotifs);
	...
	const setPreferences = async dbUser => {
		...
		if (stringPrefs) {
			...
			setTheme({
				...
				color: preferences.theme.color ? preferences.theme.color : "bg-amber-300"
			});
  ...
  `);
   ...
   `,
	...
	const filterData = (serverFolders, serverNotes, serverUser, storedData) => {
		if (storedData.folders && storedData.notes && storedData.user) {
			...
			const foldersToRemove = storedData.folders.filter(
				fold => !serverFolders.some(aFold => aFold.folderid === fold.folderid)
			);
	...
	const updateLocalDb = (serverFolders, serverNotes, storedData) => {
		const serverFoldersIdSet = new Set(serverFolders.map(fold => fold.folderid));
								...
								<>
									...
									>
										<View style={styles.tree}>
											...
											/>
											<Text
												style={[darkMode ? styles.white : styles.black, { marginTop: 10 }]}
											>
